...
local function on_attach(client, bufnr)
  ...
  vim.keymap.set('n', '<Leader>lt', require('jdtls.tests').goto_subjects, { buffer = bufnr, desc = 'Toggle between test file and source' })

  vim.keymap.set('n', '<Leader>dt', jdtls.test_nearest_method, { buffer = bufnr, desc = 'Test nearest method' })
  vim.keymap.set('n', '<Leader>dT', jdtls.test_class, { buffer = bufnr, desc = 'Test class' })

  vim.keymap.set('n', '<Leader>lo', jdtls.organize_imports, { buffer = bufnr, desc = 'Organize imports' })

  vim.keymap.set('n', '<Leader>lc', jdtls.extract_constant, { buffer = bufnr, desc = 'Extract constant' })
  vim.keymap.set('x', '<Leader>lc', function()
    jdtls.extract_constant(true)
  end, { buffer = bufnr, desc = 'Extract to constant' })

  vim.keymap.set('n', '<Leader>lv', jdtls.extract_variable_all, { buffer = bufnr, desc = 'Extract variable' })
  vim.keymap.set('x', '<Leader>lv', function()
    jdtls.extract_variable_all(true)
  end, { buffer = bufnr, desc = 'Extract to variable' })

  vim.keymap.set('x', '<Leader>lm', function()
    jdtls.extract_method(true)
  end, { buffer = bufnr, desc = 'Extract to method' })
...
local config = {
  ...
  cmd = {
    ...
    '-jar',
    vim.fn.glob(home .. '/.java/jdtls/plugins/org.eclipse.equinox.launcher_*.jar'),
    '-configuration',
    home .. '/.java/jdtls/config_linux',
    '-data',
